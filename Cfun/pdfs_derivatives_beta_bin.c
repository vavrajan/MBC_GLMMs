...
void logpYbin(struct str_state* last,  // IN last known values of generated parameters
              double* Y,                // IN [*N]
  ...
  // Can be used for either beta_bin_fix or beta_bin
  // Difference is given by pred_skip and g
  // if beta bin (group-specific coefficients) then nfix, cumnfix, FormulaF are actually grp counterparts
  ...
  *value = 0.0;
  ...
  for(l = 0; l < nUg[*g]; l++){
    ...
    for(j = 0; j < n_i[i]; j++){
      ...
      eta = 0.0;
      //if(l < 3){
      //  printf("\nlogpYbin: i = %d, j = %d, nfix = %d, eta = %f", i, j, *nfix, eta);
      //}
      for(k = 0; k < *nfix; k++){
        eta += theta[k] * X[coord + *N * FormulaF[k + *cumnfix]];
        //if(l < 3){
        //  printf("\nlogpYbin: i = %d, j = %d, k = %d, eta = %f, theta=%f, X=%f", 
      ...
      for(k = 0; k < 4; k++){
        if(*pred_skip != k){
          eta += predictor_bin[k + 4*coord];
        }
        //if(l < 3){
        //  printf("\nlogpYbin: i = %d, j = %d, k = %d, eta = %f", i, j, k, eta);
        //}
      }
      fflush(stdout);
      
      *value += Y[coord]*eta - log(1+exp(eta));
    }
  }
  
}

void logpbeta_bin(struct str_state* last,  // IN last known values of generated parameters
...
void d_d2_logpbeta_bin(struct str_state* last,  // IN last known values of generated parameters
                       double* sd_beta,          // hyperparameter - std. dev. of prior for beta
                       double* Y,                // IN [*N]
                       double* X,                // IN [N*(#regr)]    regressors
                       int* spec,            // [5]                class-specific parameters
  ...
  for(l = 0; l < nUg[*g]; l++){
    ...
    for(j = 0; j < n_i[i]; j++){
      ...
      eta = 0.0;
      //if(l < 3){
      //  printf("\nd_d2_logpbeta_bin: i = %d, j = %d, nfix = %d, eta = %f", i, j, *nfix, eta);
      //}
      for(k = 0; k < *nfix; k++){
        eta += theta[k] * X[coord + *N * FormulaF[k + *cumnfix]];
      }
      //if(l < 3){
      //  printf("\nd_d2_logpbeta_bin: i = %d, j = %d, eta = %f", i, j, eta);
      //}
      // Add other parts: G/F, R, O
      for(k = 0; k < 4; k++){
        if(*pred_skip != k){
        ...
        //if(l < 3){
        //  printf("\nd_d2_logpbeta_bin: i = %d, j = %d, k = %d, eta = %f", i, j, k, eta);
        //}
      }
      ...
      //if(l < 3){
      //  printf("\nd_d2_logpbeta_bin: i = %d, j = %d, nfix = %d, eta = %f, logitinv_eta = %f", 
      //         i, j, *nfix, eta, logitinv_eta);
      //}
      
      for(k1 = 0; k1 < *nfix; k1++){
        auxX = X[coord + *N * FormulaF[k1 + *cumnfix]];
        grad[k1] += (Y[coord] - logitinv_eta) * auxX;
        for(k2 = k1; k2 < *nfix; k2++){
          k = k1 + k2*(k2+1)/2;
          hess[k] += dli_eta * auxX * X[coord + *N * FormulaF[k2 + *cumnfix]];
        }
      }
    }
  }
