/*
 * Generating beta_bin_fix and beta_bin from the conditioned distribution 
 */
...
void metrgibbs_beta_bin(struct str_state* last,   // OUT+IN last known values of generated parameters
                        ...
                        double* prop_beta_bin_fix,      // OUT [] place for prop. distribution mean
                        double* prop_chol_beta_bin_fix, // OUT [] cholesky decomposition of proposal distributions precision matrices
                        int* cumdim_prop_beta_bin_fix,  // IN [nY[1]+1] cummulative dimensions of chol matrices
                        int* totdim_prop_beta_bin_fix,  // IN [1] total dimension of chol matrices (per one cluster)
                        double* prop_beta_bin,          // OUT [] place for prop. distribution mean
                        double* prop_chol_beta_bin,     // OUT [] cholesky decomposition of proposal distributions precision matrices
                        int* cumdim_prop_beta_bin,      // IN [nY[1]+1] cummulative dimensions of chol matrices
                        int* totdim_prop_beta_bin,      // IN [1] total dimension of chol matrices (per one cluster)
  ...
  for(y = 0; y < nY[2]; y++){
    ...
    if(nfix[shift_nY] > 0){
      ...
      if((*m % *((*tuning).freq_proposal_update)) == 0){
        ...
        //printf("\nC: Trigerring newton_raphson for beta_bin_fix: y = %d, g = %d \n", y, *G);
        newton_raphson(last, (*param).sd_beta_bin_fix, 
                       Y+*N * shift_nY, X, spec, dims, 
                       predictor_num, predictor_poi, predictor_bin+*N * 4 * y, predictor_ord, predictor_cat,
                       &pred_skip, N, n, G, Kord, Kcat, &is_cat, &kspec_bi_cat,
                       nfix+shift_nY, cumnfix+shift_nY, totnran, FormulaF, // use totnran - not used anyway
                       n_i, i_j, nUg, listUi, 
                       (*last).beta_bin_fix + beta_dim, // theta_0
                       prop_beta_bin_fix + beta_dim, // theta_max
                       prop_chol_beta_bin_fix + cumdim_prop_beta_bin_fix[y], // var_at_theta_max
                       // Functions used
                       logpbeta_bin, d_d2_logpbeta_bin,
                       // Tuning parameters
                       tuning, &iter, &converged, &bin_loglik             
        );
      }
      
      // Perform Metropolis within Gibbs
      //printf("\nC: Trigerring metropolis for beta_bin_fix: y = %d, g = %d \n", y, *G);
      metropolis(last, (*param).sd_beta_bin_fix, 
                 Y+*N * shift_nY, X, spec, dims, 
                 predictor_num, predictor_poi, predictor_bin+*N * 4 * y, predictor_ord, predictor_cat,
                 &pred_skip, N, n, G, Kord, Kcat, &is_cat, &kspec_bi_cat,
                 nfix+shift_nY, cumnfix+shift_nY, totnran, FormulaF, // use totnran - not used anyway
                 n_i, i_j, nUg, listUi, 
                 (*last).beta_bin_fix + beta_dim, // theta_0
                 prop_chol_beta_bin_fix + cumdim_prop_beta_bin_fix[y], // 
                 // Functions used
                 logpbeta_bin,
                 // Tuning parameters
                 tuning, (*tuning).const_proposal_beta_bin_fix
      );
  ...
  ////-----------------------------------////
  //// Group-specific effects - beta_bin ////
  ////-----------------------------------////
  pred_skip = 1;
  beta_dim = 0;
  
  for(g = 0; g < *G; g++){
    // First by g because these g-blocks of betas for Bins outcomes are stored after each other
    shift_nY = nY[0]+nY[1];
    for(y = 0; y < nY[2]; y++){
      // UPDATE AND PROPOSE ONLY WHEN THERE IS SOMETHING TO BE PROPOSED!
      if(ngrp[shift_nY] > 0){
        ...
        if((*m % *((*tuning).freq_proposal_update)) == 0){
          ...
          //printf("\nC: Trigerring newton_raphson for beta_bin: y = %d, g = %d \n", y, g);
          newton_raphson(last, (*param).sd_beta_bin, 
                         Y+*N * shift_nY, X, spec, dims, 
                         predictor_num, predictor_poi, predictor_bin+*N * 4 * y, predictor_ord, predictor_cat,
                         &pred_skip, N, n, &g, Kord, Kcat, &is_cat, &kspec_bi_cat,
                         ngrp+shift_nY, cumngrp+shift_nY, totnran, FormulaG, // use totnran - not used anyway
                         n_i, i_j, nUg, listUi, 
                         (*last).beta_bin + beta_dim, // theta_0
                         prop_beta_bin + beta_dim, // theta_max
                         prop_chol_beta_bin + cumdim_prop_beta_bin[y] + *totdim_prop_beta_bin * g, // var_at_theta_max
                         // Functions used
                         logpbeta_bin, d_d2_logpbeta_bin,
                         // Tuning parameters
                         tuning, &iter, &converged, &bin_loglik             
        ...
        // Perform Metropolis within Gibbs
        //printf("\nC: Trigerring metropolis for beta_bin: y = %d, g = %d \n", y, g);
        metropolis(last, (*param).sd_beta_bin, 
                   Y+*N * shift_nY, X, spec, dims, 
                   predictor_num, predictor_poi, predictor_bin+*N * 4 * y, predictor_ord, predictor_cat,
                   &pred_skip, N, n, &g, Kord, Kcat, &is_cat, &kspec_bi_cat,
                   ngrp+shift_nY, cumngrp+shift_nY, totnran, FormulaG, // use totnran - not used anyway
                   n_i, i_j, nUg, listUi, 
                   (*last).beta_bin + beta_dim, // theta_0
                   prop_chol_beta_bin + cumdim_prop_beta_bin[y] + *totdim_prop_beta_bin * g, // 
                   // Functions used
                   logpbeta_bin,
                   // Tuning parameters
                   tuning, (*tuning).const_proposal_beta_bin
        );
